...
let toConsolidate = 'NamedEvaluation';
let consolidateInto = 'sec-syntax-directed-operations-function-name-inference';
...
async function go() {
  ...
  for (let grammar of document.querySelectorAll('emu-grammar')) {
    if (grammar.getAttribute('type') === 'definition' || grammar.hasAttribute('example')) {
      continue;
    }
  ...
  for (let { alg, grammar, isInText, isInAnnexB } of sdos) {
    ...
    if (name(pointer) !== 'emu-grammar') {
      throw new Error('unexpected node: ' + name(pointer) + ' in ' + sdoName);
    }
  ...
  /*
  let singles = clauses.filter(p => p[1].length === 1);
  let todo = [];
  for (let single of singles) {
    let loc = dom.nodeLocation(single[1][0].ele);
    let openTag = source.substring(loc.startTag.startOffset, loc.startTag.endOffset);
    if (!/type="sdo"/.test(openTag) || !/aoid="/.test(openTag)) {
      todo.push({ offset: loc.startTag.endOffset - 1, text: ' type="sdo" aoid="' + single[0] + '"' });
    }
  }
  todo.sort(((a, b) => b.offset - a.offset));
  for (let { offset, text } of todo) {
  ...
  */
  ...
  let targetClause = clauses.find(c => c[0] === toConsolidate);
  if (targetClause == null) {
    throw new Error('could not find clause');
  }

  let [, eles] = targetClause;
  let first = true;
  let newLines = [];
  let footer = [];
  let toRemove = [];
  ...
  let oldIds = eles.flatMap(e => [e.ele.id].concat((e.ele.getAttribute('oldids') ?? '').split(',').filter(c => c.length > 0))).join(',');
  let openTag = `    <emu-clause id="${makeId(eles[0].ele.querySelector('h1').innerHTML)}" oldids="${oldIds}" type="sdo" aoid="${eles[0].name}">`;
  ...
  // rather than figuring out how to remove and add text in the same pass, just re-parse
  dom = new jsdom.JSDOM(source, { includeNodeLocations: true });
  document = dom.window.document;
  let targetEle = document.querySelector('#' + consolidateInto);
  if (targetEle == null) {
    throw new Error('could not find clause ' + consolidateInto);
  }
  let targetLoc = dom.nodeLocation(targetEle);
  let targetOffset = targetLoc.endTag.startOffset - targetLoc.endTag.startCol + 1;
  source = source.substring(0, targetOffset) + '\n' + newContents + source.substring(targetOffset);
  fs.writeFileSync('./spec.html', source, 'utf8');
  console.log('success!');
...
function skipHeader(pointer) {
  switch (name(pointer)) {
    case 'p': {
      if (/^With( optional)? parameter/.test(pointer.innerHTML)) {
